...
int** aStar(int** matrix, int start[], int goal[]) {
    ...
    fScore[start[0]] = heuristicEuclidian(start); // Coût total estimé du début au but en passant par le noeud de départ
    ...
    while (openListSize > 0) {
        ...
        if (currentNode[0] == goal[0]) {
            ...
            while (tempNode != start[0]) {
                ...
                for (int j = 0; j < 7; j++) {
                    path[pathSize][j] = nodes[tempNode][j];
                }
                pathSize++;
                tempNode = closedList[tempNode][1]; // Parent node
            }
            path[pathSize] = (int*)malloc(7 * sizeof(int));
            for (int j = 0; j < 7; j++) {
        ...
        int neighbors[4] = {currentNode[4], currentNode[5], currentNode[6], currentNode[7]}; // left, right, up, down
        for (int i = 0; i < 4; i++) {
            if (neighbors[i] != -1 && neighbors[i] != 0) { // Vérifier si le voisin est accessible
                int neighborNode[7];
                for (int j = 0; j < 7; j++) {
                ...
                if (tentativeGScore < gScore[neighborNode[0]]) {
                    ...
                    fScore[neighborNode[0]] = gScore[neighborNode[0]] + heuristicEuclidian(neighborNode);
